...
def get_tracked_objects():
    ...
    # SATCAT totals (from tracked_total.json generated by update_tracked_totals.py)
    ...
    # Simple derived estimate: if total known, estimate "not active satellites"
    inactive_est = max(0, tracked_total - active_count) if tracked_total > 0 else 0

    return TrackedObjectsSummary(
        data_source=str(base.get("data_source", "CelesTrak SATCAT Boxscore")),
